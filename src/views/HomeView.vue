    ...
    <main class="relative z-10 w-full max-w-lg h-[95vh] md:h-[88vh]
      ...
      <transition name="timeline-slide">
        <div v-if="showTimeline"
          ...
          <div class="shrink-0 flex items-center justify-between px-4 py-3 border-b border-tao-gold/20">
            ...
            <button @click="showTimeline=false" class="font-mono text-white/30 hover:text-white/70 transition-colors"
      ...
      <section v-if="step==='intro'" class="flex-1 flex flex-col items-center justify-between w-full py-4 px-5 overflow-hidden">
        <header class="text-center w-full animate-fade-in shrink-0 flex flex-col items-center">
          <h1 class="font-serif text-white neon-text-gold" style="font-size:28px;letter-spacing:0.55em;margin-bottom:4px">CYBER TAO</h1>
        ...
        <div class="w-full flex-1 flex flex-col justify-center gap-3">
          ...
          <div class="flex flex-wrap gap-2">
            <button v-for="tpl in questionTemplates" :key="tpl.label" @click="applyTemplate(tpl)"
          ...
          <SpiritBottle :spiritPoints="spiritPoints" :isUnlimited="isAdmin" :shareCount="shareCount" @refill="handleRefillShare" />
        </div>

        <button @click="step='ritual'" :disabled="!question.trim() || (!hasSpirit && !isAdmin)"
      ...
      <section v-else-if="step==='result'" class="flex-1 flex flex-col w-full overflow-hidden">
        ...
        <div v-else class="flex-1 flex flex-col overflow-y-auto custom-scrollbar animate-fade-in-up px-4 pt-3 pb-2 gap-3">
          ...
          <!-- 变爻横幅 -->
          <div v-if="hasChangingLine" class="shrink-0 flex items-center gap-2 px-3 py-2 border"
          ...
          <div v-if="aiOracle" class="relative cyber-terminal-box shrink-0">
            ...
            <div class="p-4 text-center">
              <div class="flex items-center gap-3 mb-3">
                ...
                <span style="color:rgba(200,170,110,0.45);font-size:10px">◈</span>
              ...
              <p class="font-serif leading-relaxed"
                 style="font-size:15px;color:rgba(255,255,255,0.92);line-height:1.85;letter-spacing:0.04em;text-shadow:0 0 18px rgba(200,170,110,0.15)">
                {{ aiOracle }}
              </p>
          ...
          <div class="relative cyber-terminal-box shrink-0">
            ...
            <div class="p-4">
              ...
              <!-- 未解锁 -->
              <div v-if="!fullAnalysisUnlocked">
                <div class="relative overflow-hidden rounded-sm" style="min-height:72px">
                  <p class="font-mono italic leading-relaxed whitespace-pre-wrap select-none"
                    ...
                    {{ aiResult.slice(0,260) }}
                  </p>
                  <div class="absolute inset-0" style="background:linear-gradient(to bottom,rgba(12,16,24,0.2) 0%,rgba(12,16,24,0.7) 45%,rgba(12,16,24,0.97) 100%)"></div>
                ...
                <div class="flex flex-col items-center mt-3 gap-2">
                  ...
                  <button v-if="spiritPoints >= 8 || isAdmin"
                    @click="unlockFullAnalysis" :disabled="unlockLoading"
                  ...
                  <button v-else @click="showRechargeHint=!showRechargeHint"
                    class="w-full border font-mono font-black uppercase transition-all cyber-btn-corners relative"
                    style="padding:10px 0;font-size:11px;letter-spacing:0.35em;border-color:rgba(239,68,68,0.45);color:rgba(239,68,68,0.75);background:rgba(239,68,68,0.05)">
                    ⚡ INSUFFICIENT SPIRIT
                  </button>
              ...
              <div v-else class="animate-fade-in">
                ...
                <p class="text-gray-100 font-mono italic leading-relaxed whitespace-pre-wrap" style="font-size:12px;line-height:1.7">
                  {{ aiResult }}
                </p>
        ...
        <div class="shrink-0 grid grid-cols-2 gap-3 px-4 py-3 border-t border-tao-gold/25">
          <button @click="sealDestiny" :disabled="sealLoading"
...
const fullAnalysisUnlocked = ref(false)
...
const dailyReadings = ref(0)
const MAX_SHARES_PER_DAY = 3

// ── 地理 ──
const geoRegion = ref('VOID')
...
const initIdentity = async () => {
  ...
  if (!id) {
    ...
    await supabase.from('device_profiles').insert([{ device_id: id, spirit_points: MAX_SPIRIT }])
  }
  deviceId.value = id
  const { data } = await supabase.from('device_profiles')
    .select('is_unlimited,spirit_points,last_reading_date,daily_readings,share_count,last_share_date')
    .eq('device_id', id).single()
  if (data) {
    isAdmin.value = data.is_unlimited ?? false
    spiritPoints.value = data.spirit_points ?? MAX_SPIRIT
...
const unlockFullAnalysis = async () => {
  ...
  try {
    const newSP = Math.max(0, spiritPoints.value - SPIRIT_PER_UNLOCK)
    await supabase.from('device_profiles').update({ spirit_points: newSP }).eq('device_id', deviceId.value)
    spiritPoints.value = newSP
...
const onRitualComplete = async (payload) => {
  ...
  changingLinesArr.value = changingLines
  step.value = 'result'
  loading.value = true
  ...
  try {
    ...
    const newSP = isAdmin.value ? spiritPoints.value : Math.max(0, spiritPoints.value - SPIRIT_PER_READ)
    spiritPoints.value = newSP
...
const handleRefillShare = async () => {
  ...
  try {
    if (navigator.share) {
      ...
      const newSP = Math.min(MAX_SPIRIT, spiritPoints.value + SPIRIT_PER_SHARE)
      await supabase.from('device_profiles').update({
        share_count: nc, last_share_date: today, spirit_points: newSP,
      }).eq('device_id', deviceId.value)
      shareCount.value = nc
      spiritPoints.value = newSP
    }
...
const sealDestiny = async () => {
  ...
  try {
    ...
    const { error } = await supabase.from('divination_logs').update({
      card_id: cardId, edition_number: editionNumber,
      verified_hash: verifiedHash, oracle: aiOracle.value, is_sealed: true,
    }).eq('device_id', deviceId.value).eq('hexagram_code', hexCode).is('card_id', null)
...
<style scoped>
.neon-text-gold { text-shadow: 0 0 8px rgba(200,170,110,0.95), 0 0 20px rgba(200,170,110,0.55), 0 0 45px rgba(200,170,110,0.28); }
.neon-text-white { text-shadow: 0 0 10px rgba(255,255,255,0.65), 0 0 25px rgba(200,170,110,0.38), 0 0 50px rgba(200,170,110,0.18); }
.panel-frame {
...
.panel-frame::before { content:''; position:absolute; top:0; left:0; width:18px; height:18px; z-index:20; pointer-events:none; border-top:2px solid rgba(200,170,110,0.88); border-left:2px solid rgba(200,170,110,0.88); box-shadow:-2px -2px 10px rgba(200,170,110,0.45); }
.panel-frame::after  { content:''; position:absolute; bottom:0; right:0; width:18px; height:18px; z-index:20; pointer-events:none; border-bottom:2px solid rgba(200,170,110,0.88); border-right:2px solid rgba(200,170,110,0.88); box-shadow:2px 2px 10px rgba(200,170,110,0.45); }
.cyber-terminal-box { background:rgba(0,0,0,0.5); border:1px solid rgba(200,170,110,0.38); border-radius:2px; position:relative; box-shadow:inset 0 0 18px rgba(200,170,110,0.04), 0 0 12px rgba(200,170,110,0.07); }
.cyber-terminal-box::before { content:''; position:absolute; top:0; left:0; width:10px; height:10px; border-top:2px solid rgba(200,170,110,0.92); border-left:2px solid rgba(200,170,110,0.92); }
.cyber-terminal-box::after  { content:''; position:absolute; bottom:0; right:0; width:10px; height:10px; border-bottom:2px solid rgba(200,170,110,0.92); border-right:2px solid rgba(200,170,110,0.92); }
.cyber-btn-corners::before { content:''; position:absolute; top:-1px; left:-1px; width:7px; height:7px; border-top:2px solid #c8aa6e; border-left:2px solid #c8aa6e; }
.cyber-btn-corners::after  { content:''; position:absolute; bottom:-1px; right:-1px; width:7px; height:7px; border-bottom:2px solid #c8aa6e; border-right:2px solid #c8aa6e; }
...
.cyber-grid-visible { background-image:linear-gradient(rgba(200,170,110,0.11) 1px,transparent 1px),linear-gradient(90deg,rgba(200,170,110,0.11) 1px,transparent 1px); background-size:18px 18px; }
