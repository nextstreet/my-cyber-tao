...
export function ganzhiYear(year: number): StemBranch {
  ...
  return { stem: HEAVENLY_STEMS[stemIdx]!, branch: EARTHLY_BRANCHES[branchIdx]!, zodiac: ZODIAC_ANIMALS[branchIdx]!, zodiacEn: ZODIAC_EN[branchIdx]! }
}
...
export type LunarMansion = { index: number; symbol: string; name: string; nameEn: string; element: string; elementEn: string; direction: '东'|'南'|'西'|'北'; directionEn: 'East'|'South'|'West'|'North'; animal: string; animalEn: string }
