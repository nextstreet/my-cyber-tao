...
export function castLine(): LineValue {
  const coins = [Math.random() > 0.5 ? 3 : 2, Math.random() > 0.5 ? 3 : 2, Math.random() > 0.5 ? 3 : 2] as const
  return (coins[0] + coins[1] + coins[2]) as LineValue
}

export function lineIsYang(v: LineValue): boolean { return v === 7 || v === 9 }
export function lineIsChanging(v: LineValue): boolean { return v === 6 || v === 9 }
...
export function linesToHexagram(lines: LineValue[]): Hexagram {
  ...
  return { index: clampedIdx + 1, symbol: sym, nameZh, nameEn, judgment, judgmentEn, image: `${upperT.el} above ${lowerT.el}` }
}
