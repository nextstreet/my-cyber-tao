  ...
  <div class="fixed inset-0 bg-tao-black text-tao-gold flex flex-col items-center justify-center p-4 overflow-hidden">
    ...
    <main class="z-10 w-full max-w-md bg-black/60 backdrop-blur-2xl p-8 border border-tao-gold/10 relative overflow-hidden">
      <section v-if="step === 'intro'" class="flex flex-col items-center space-y-8 animate-fade-in">
        ...
        <button @click="startRitual" :disabled="!question || !hasSpirit" class="w-full py-4 border border-tao-gold text-xs tracking-widest uppercase hover:bg-tao-gold hover:text-black transition-all">ENTER RITUAL</button>
      </section>
      ...
      <section v-else-if="step === 'result'" class="relative flex flex-col items-center min-h-[420px] justify-between">
        ...
        <div class="z-10 text-center w-full mt-2">
          ...
          <div v-if="loading" class="py-16 animate-pulse text-[10px] tracking-[0.6em] text-tao-gold/60">SYNCHRONIZING DAO...</div>
        ...
        <div v-if="!loading" class="grid grid-cols-2 gap-4 w-full mt-8 z-10">
          <button @click="showTalisman" class="py-4 bg-tao-gold text-black text-[10px] font-bold tracking-widest uppercase">EXTRACT</button>
          <button @click="reset" class="py-4 border border-tao-gold/20 text-[10px] text-tao-gold/40 uppercase">RETURN</button>
        </div>
      </section>
    </main>
...
const onRitualComplete = async (lines) => {
  ...
  try {
    ...
    aiResult.value = 'SIGNAL INTERRUPTED: ' + err.message
